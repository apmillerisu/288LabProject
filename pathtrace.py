        ...
        for part in parts:
            ...
            if len(key_value) == 2:
                ...
                if key == "BUMP_L" and value_str == '1': left_bumper_color = "red"
                    ...
                    heading_val_str = value_str # Keep as string, Python doesn't use this for logic
    except Exception as e:
...
def detect_and_plot_objects(scan_data=None):
    ...
    # Use the static robot_x, robot_y for sensor origin calculation if map icon is static
...
def update_map_with_bump(bump_info_string):
    ...
    # Use static robot_x, robot_y for bump event plotting relative to static icon
...
# In your Python GUI script (SomewhatWorkingGUI.py)
...
def update_robot_position_and_trail(move_data_string):
    ...
    """
    # No longer needs global robot_angle_deg as it doesn't modify it
    ...
    try:
        ...
        for part in parts:
            ...
            if len(key_value) == 2:
                ...
                if key == "DIST_CM":
                    ...
                    angle_deg_delta = float(value) # Parse the angle delta for the trail history

        # if DEBUG_TRAIL_PANEL:
        #     print(f"TRAIL_DEBUG: update_robot_pos_and_trail received: dist_cm={dist_cm}, angle_delta={angle_deg_delta}")

        # ---vvv--- Lines REMOVED that updated global robot_angle_deg ---vvv---
        # robot_angle_deg += angle_deg_delta
        # robot_angle_deg %= 360
        # if robot_angle_deg < 0: robot_angle_deg += 360
        # ---^^^-----------------------------------------------------------^^^---

        # ---vvv--- Line REMOVED that called draw_robot_on_map ---vvv---
        # We don't need to redraw the static map icon every time a move message comes in
        # draw_robot_on_map()
        # ---^^^----------------------------------------------------^^^---


        # --- Add to movement history for the new trail panel ---
        # This still uses the parsed angle_deg_delta correctly
        if abs(angle_deg_delta) > 0.01: # Store turn if significant
        ...
        if abs(dist_cm) > 0.01: # Store move if significant
...
def clear_map_features(tag_to_clear):
    ...
    # No need to call draw_robot_on_map or initialize_robot_position if map trail is not used.
    # If objects/bumps are cleared, the main map icon remains static.

def draw_robot_on_map(event=None): 
    global robot_x, robot_y, robot_angle_deg, map_canvas, MAP_SCALE 
    if not map_canvas: return
    map_canvas.delete("robot") 
    if not is_connected and not (robot_x > 0 and robot_y > 0 ):
        return
    radius_pixels = ROBOT_REAL_RADIUS_CM * MAP_SCALE
    cx, cy = robot_x, robot_y # Uses the (potentially static) robot_x, robot_y
    x1 = cx - radius_pixels
    y1 = cy - radius_pixels
    x2 = cx + radius_pixels
    y2 = cy + radius_pixels
...
def clear_all_trails():
    """Clears movement trail panel and recenters robot icon on the main map."""
    global map_canvas 
    if DEBUG_TRAIL_PANEL: print("TRAIL_DEBUG: clear_all_trails called")
    
    # Recenter robot icon on the main map (map trail is no longer drawn by update_robot_position_and_trail)
    if map_canvas:
        map_canvas.delete("trail") # Clear any old map trails if they existed from previous versions
    initialize_robot_position() 
    
    initialize_trail_display() 
...
def redraw_trail_on_panel(event=None): 
    ...
    if DEBUG_TRAIL_PANEL:
        ...
        if trail_canvas:
            # Forcing update of dimensions before reading them
            trail_canvas.update_idletasks() 
    ...
    try:
        ...
        trail_canvas.create_oval(current_pen_x - 3, current_pen_y - 3, current_pen_x + 3, current_pen_y + 3, 
                                 fill="blue", outline="blue", tags="trail_segment")
        ...
        for i, movement in enumerate(movement_history):
            ...
            if movement['type'] == 'turn':
                current_pen_angle_deg -= movement['angle']
